...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const type = validateEnum(body.type, COLOR_LIMITS.harmony_types, 'type')

    let format = body.format
    if (format) {
      format = validateEnum(format, COLOR_LIMITS.harmony_formats, 'format')
    }

    // Generate harmony
    const result = generateHarmony({ input, type, format })
    ...
    console.error('Harmony endpoint error:', error)
    return errorResponse(
      'INTERNAL_ERROR',
